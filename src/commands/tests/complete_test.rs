...
async fn given_existing_tasks_when_completing_a_task_then_the_other_parts_of_the_task_should_stay_the_same(
    ...
    let _: Option<Task> = db
        .client
        .insert(("task", "task to complete"))
        .content(Task {
            name: "task to complete".to_string(),
            priority: TaskPriority::Medium.to_string(),
            description: Some("This is a description of the task".to_string()),
            created_at: Default::default(),
            completed_at: None,
        })
        .await
    ...
    let res = complete::run(&db, "task to complete".to_string()).await;
    assert!(res.is_ok());
    let res_str = res.unwrap();
    assert_eq!(
        res_str,
        "Successfully updated task 'task to complete' to completed".to_string()
    );
